...
public class Solver1 extends Thread {
	private Thread t;
	...
	public Solver1(String name, int[][] sudokuTable, int startX, int startY) {
		threadName = name;
		this.startX = startX;
		this.startY = startY;
		this.sudokuTable = sudokuTable;
		...
		if (sudokuTable[i][j] != 0)
			...
			for (int x = 1; x < 10; x++) {
				if (!doesRowIncludeThisValue(i, j, x) && !doesColumnIncludeThisValue(i, j, x)
					...
					sudokuTable[i][j] = x;
					try (FileWriter fw = new FileWriter("thread1.txt", true);
